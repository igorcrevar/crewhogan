...
module.exports = function(baseDir, debug) {
	...
	function getPathFromMatches(matches) {
		var paths = baseDir ? [baseDir] : [];
	...
	function getTemplate(name, partials){
		...
		if (!cached) {
			...
			if (matches) {
				var filePath = getPathFromMatches(matches);
				// console.log("resolve " + name + " " + filePath);
				var data = readFile(filePath);

				// compile template and store it to cached templates
				cachedTemplates[name] = cached = { template: hogan.compile(data), partials: {} };
